...
fetch_latest <- function(fileDir, type = "cases"){
  ...
  if(type == "even_days"){
    ...
    # opts = opts[grepl(".RData", opts[, 2]), ]
    # # trim to date part
    # # opts = opts[grepl("BigStandard_even_days_", opts[, 2]), ]
    # opts[, 2] = gsub("BigStandard_even_days_", "", opts[, 2])
    # opts[, 2] = gsub(".RData", "", opts[, 2])
    # opts[, 2] = gsub("_", "", opts[, 2])
    # # to date
    # opts[, 2] = as.Date(opts[, 2], "%Y%m%d")
    ...
    # opts = opts[order(opts[, 2]), ]
    # load and return
    return(file.path(fileDir, "BigStandard_even_days.RData"))
  }
  
  if(type == "red_size"){
    return(file.path(fileDir, "Brazil_BigStandard_results_red_size2021_04_15.RData"))
  }
}
